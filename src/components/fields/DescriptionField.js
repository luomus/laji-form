...
export default function DescriptonField(props) {
	...
	const _description = replacePatterns.reduce((desc, replacePattern) => {
		...
		let replacement;
		if (replacePattern[0] === "#") {
			replacement = parseJSONPointer(new Context(props.formContext.contextId).formData, replacePattern.substr(1));
		...
		//	replacement = parseJSONPointer(props.formData, replacePattern);
			...
			const value = parseJSONPointer(new Context(props.formContext.contextId).formData, field.substr(1));
			const bounds = L.geoJSON(value).getBounds();
			const sw = bounds.getSouthWest();
			const ne = bounds.getNorthEast(); 
			replacement = `${sw.lat}:${ne.lat}:${sw.lng}:${ne.lng}:WGS84`;
		} else if (replacePattern === "lajiFiBase") {
			replacement = props.formContext.lajiFiBase;
