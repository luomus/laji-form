...
export default class UnitShorthandField extends React.Component {
	...
	shouldShowSchema = (props) => {
		...
		if (this.state.showSchema === undefined && !this.props.formContext.utils.formDataIsEmpty(props)) {
			showSchema = true;
		}
		return showSchema;
	}
	...
	onCodeChange = (formData = {}) => {
		...
		this.onNextTick = () => {
			if (autocopy) {
				this.props.formContext.services.customEventService.send(this.props.idSchema.$id, "copy");
			} else if (autofocus) {
				this.props.formContext.services.customEventService.send(this.props.idSchema.$id, "focus", "last");
			}
