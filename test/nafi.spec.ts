...
import { DemoPageForm, createForm, TaxonAutosuggestWidgetPO } from "./test-utils";
...
test.describe("NAFI (MHL.6)", ()  => {
	...
	test.describe("selecting species name with mouse", () => {
		...
		test("selects suggestion value", async () => {
			...
			await expect(taxonAutosuggest.$suggestionsContainer).toBeVisible(); // Wait for suggestions.
